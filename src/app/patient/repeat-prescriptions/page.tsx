      ...
      <div className="p-4">
        <div className="bg-[#0B7A82] rounded-3xl p-5 mb-5 text-white">
          ...
          <p className="text-cyan-100 text-sm">
            Order your regular medication — usually takes 2 working days to
            process.
          </p>
        </div>
        ...
        <div className="space-y-3 mb-6">
          {methods.map(({ Icon, title, body, dark, bg }) => (
            <div
              key={title}
              className={`rounded-2xl p-4 ${dark ? `${bg} text-white` : "bg-white border border-border"}`}
            >
              <div className="flex items-start gap-3">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${dark ? "bg-white/20" : "bg-[#0B7A82]/10"}`}
                >
                  <Icon
                    className={`w-5 h-5 ${dark ? "text-white" : "text-[#0B7A82]"}`}
                  />
                </div>
                <div>
                  <p
                    className={`font-semibold text-sm mb-1 ${dark ? "text-white" : "text-dark"}`}
                  >
                    {title}
                  </p>
                  <p
                    className={`text-sm leading-relaxed ${dark ? "text-green-100" : "text-text-secondary"}`}
                  >
                    {body}
                  </p>
                </div>
